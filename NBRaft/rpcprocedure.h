...
namespace Nano {
	namespace Rpc {
		typedef std::function<void(Json::Value response)> ProcedureDoneCallback;				// 过程执行完成后的回调函数
		typedef std::function<void(Json::Value&, const ProcedureDoneCallback&)> ReturnFunc;		// 处理完成需要回调响应的函数
		typedef std::function<void(Json::Value&)> NotifyFunc;									// 处理完成不需要回调响应的函数
